...
| BASI text (`.txt` or old BASI-Epsilon `.dat`) | CV | Supported | Load, class promotion to `cv`, metadata/units parsing, plot and peak-analysis compatible object behavior | Header/unit variants beyond tested fixtures may need parser fixes; headerless old `.dat` CV exports are interpreted as Potential/Current in V/A | Recommended if the export resembles tested text files |
