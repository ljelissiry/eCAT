...
- `metadata`: setup metadata such as scan rate, segments, quiet time, units,
  stride, and source-window details
...
- `show(options=None)`: displays setup by default. `{"print params": True}`
  ...
  parameter checks, and `{"print data": True}` adds the full simulated data
  table.
...
- `mechanism`: backend mechanism string
...
- `raw`: whether the user supplied a raw backend mechanism string
