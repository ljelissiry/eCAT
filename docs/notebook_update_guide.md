...
- In notebook 03, plotted peak-potential and peak-current analysis diagnostics
  on one graph.
- Added beginner examples for `peak_width()`, `half_peak_potential()`,
  `peak_info()`, `half_wave_potential()`, and `wave_info()` using a single
  catalytic trace and a cleaner Fc/Fc+ paired-wave trace.
...
- Use notebook 04 for focused plotting mechanics: `cv.trim()` for one trace,
  ...
  `gradient reverse`, `gradient scale` including the `"auto"` log default for
  scan-rate and positive concentration gradients, `multimultiplot()`, and
  saving polished figures.
...
- Put scan-rate-series reversibility in notebook 07, after individual CV wave
  metrics and before advanced fitting. Use bundled real data for both the Fc
  reversible-wave example and the Fe quasi-reversible example.
- Keep surface-confined workflows separate in notebook 08. Import the checked-in
  `surface_confined_cv_series.xlsx` workbook rather than generating it inside the
  notebook; the generation script and provenance live beside the fixture under
  `examples/data/surface_coverage_cv/`.
- Move normalization workflows to notebook 09: physical dimensionless
...
- Keep advanced fitting/statistics material in notebook 09 and later. Simulation,
  group fitting, and equilibria fitting begin in notebooks 10, 11, and 12.
...
- Avoid advanced fitting/statistics terminology until notebook 09; notebook 07
  may introduce only the series-level kinetic evidence needed to explain
  reversibility, and notebook 08 remains surface-specific.
