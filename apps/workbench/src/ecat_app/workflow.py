"""Structured workflow state for the eCAT app."""
...
class AppWorkflow:
    ...
    analysis_actions: list[dict[str, object]] = field(default_factory=list)
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "analysis_actions": [dict(action) for action in self.analysis_actions],
    ...
    @classmethod
    def from_dict(cls, data: dict[str, object] | None) -> "AppWorkflow":
        if not data:
            return cls()
        return cls(
            ...
            analysis_actions=[dict(action) for action in data.get("analysis_actions") or []],
